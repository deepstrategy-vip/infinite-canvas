...
+ [新增] 视频生成支持直接使用本地素材：拖入画布的图片、参考视频和参考音频会在生成时自动上传并换成公网地址，用户不需要自己准备对象存储或 CDN。
+ [修复] 本地图片连接视频节点时因素材被内联为 Base64 而创建任务失败的问题。

## v0.9.0-onetoken.4 - 2026-07-24

+ [修复] OneToken 连接统一填写根地址，内置调用脚本会按能力正确选择 `/v1`、`/v1beta` 或 `/api/v3`，并兼容旧版带路径的配置。
+ [修复] 文本、图片、音频和视频内置调用模板统一限定为 OneToken 公共 API，移除客户界面的内部服务来源与直连说明。
+ [文档] 补齐 Seedance 2.0 创建任务、轮询结果、多模态素材、完整参数和幂等键使用教程。
+ [修复] OneToken 公开版 Docker Compose 改为从当前源码构建，避免误拉取不含 OneToken 模板的上游镜像。
+ [新增] 视频模型调用脚本新增 OneToken Seedance 2.0 模板，统一通过 OneToken 公共 API 创建并查询任务。
+ [调整] 自定义视频脚本可读取参考视频和参考音频，并对 Seedance 参数和素材限制进行统一校验。
...
+ [新增] 支持复制 OneToken 生图/视频模板并自定义请求参数与结果解析。
...
+ [修复] 兼容 OneToken 视频任务直接返回视频 URL 的情况，并优化失败原因展示。

## v0.5.0 - 2026-07-05

+ [新增] OneToken 连接兼容 Gemini 原生格式。
